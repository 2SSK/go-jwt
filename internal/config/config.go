...
func LoadConfig() (*Config, error) {
	...
	for _, env := range os.Environ() {
		...
		if len(parts) == 2 && strings.HasPrefix(parts[0], "Auth_") {
			...
			configKey := strings.ToLower(strings.TrimPrefix(key, "Auth_"))
	...
	err := k.Load(env.ProviderWithValue("JWT_", ".", func(key, value string) (string, any) {
		return strings.ToLower(strings.TrimPrefix(key, "JWT_")), value
	}), nil)
	if err != nil {
	...
	mainConfig.Observability.ServiceName = "jwt"
